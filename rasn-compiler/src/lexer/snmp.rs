...
#[derive(Debug, Clone, PartialEq)]
pub struct TextualConvention<'i> {
...
pub fn textual_convention(input: Input<'_>) -> ParserResult<'_, TextualConvention> {
    preceded(
        ...
        map(
            cut(tuple((
                skip_ws_and_comments(opt(preceded(tag("DISPLAY-HINT"), character_string_value))),
    ...
    preceded(
        ...
        map(
            tuple((
                ...
                skip_ws_and_comments(opt(preceded(tag("DEFVAL"), asn1_value))),
            )),
            |v| ObjectType {
                syntax: v.0,
                units: v.1,
...
pub struct NotificationType<'i> {
    // OBJECTS { value(ObjectName) , .. } | empty
    ...
    preceded(
        ...
        map(
            tuple((
                ...
                skip_ws_and_comments(preceded(tag("DESCRIPTION"), character_string_value)),
                skip_ws_and_comments(opt(preceded(tag("REFERENCE"), character_string_value))),
            )),
            |v| NotificationType {
...
pub fn module_compliance(input: Input<'_>) -> ParserResult<'_, ModuleCompliance> {
    preceded(
        ...
        map(
            cut(tuple((
                preceded(
                    skip_ws_and_comments(tag("STATUS")),
                    skip_ws_and_comments(value_identifier),
                ),
                preceded(
                    skip_ws_and_comments(tag("DESCRIPTION")),
                ...
                skip_ws_and_comments(many1(module_compliance_module)),
            ))),
            |v| ModuleCompliance {
                status: v.0,
                description: v.1,
