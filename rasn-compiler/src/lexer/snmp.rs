...
use crate::intermediate::{
    ASN1Type, ASN1Value, MANDATORY_GROUPS, MODULE_COMPLIANCE, MODULE_IDENTITY, NOTIFICATION_GROUP,
    NOTIFICATION_TYPE, OBJECT_GROUP, OBJECT_TYPE,
};
use crate::lexer::character_string::{cstring, raw_string_literal};
use crate::lexer::common::{in_braces, module_reference, skip_ws_and_comments, value_reference};
...
pub fn module_identity(input: Input<'_>) -> ParserResult<'_, ModuleIdentity> {
    preceded(
        skip_ws_and_comments(tag(MODULE_IDENTITY)),
        map(
            (
                skip_ws_and_comments(preceded(tag("LAST-UPDATED"), skip_ws_and_comments(cstring))),
                skip_ws_and_comments(preceded(tag("ORGANIZATION"), skip_ws_and_comments(cstring))),
                skip_ws_and_comments(preceded(tag("CONTACT-INFO"), skip_ws_and_comments(cstring))),
                skip_ws_and_comments(preceded(tag("DESCRIPTION"), skip_ws_and_comments(cstring))),
    ...
    map(
        (
            preceded(
                ...
                skip_ws_and_comments(skip_ws_and_comments(cstring)),
            ),
            preceded(
        ...
        map(
            cut((
                skip_ws_and_comments(opt(preceded(
                    tag("DISPLAY-HINT"),
                    cut(skip_ws_and_comments(cstring)),
                ))),
...
pub fn object_type_index(input: Input<'_>) -> ParserResult<'_, ObjectTypeIndex> {
    alt((
            ...
            map(
                cut(in_braces(separated_list1(
                    ...
                    (
                        ...
                        skip_ws_and_comments(raw_string_literal),
                    ),
                ))),
        ...
        preceded(
            ...
            map(
                cut(in_braces(skip_ws_and_comments(value_reference))),
...
pub enum Compliance<'i> {
    ...
    Object {
        ...
        //   | "BITS" "{" identifier "(" number ")" ( "," identifier "(" number ")" ) * "}"
        //   | empty
        write_syntax: Option<Box<ASN1Type>>,
        ...
        map(
            cut((
                opt((
                    ...
                    skip_ws_and_comments(opt(value_reference)),
                )),
                opt(preceded(
        ...
        preceded(
            ...
            map(
                cut((
                    ...
                    skip_ws_and_comments(opt(preceded(
                        tag("MIN-ACCESS"),
                        cut(skip_ws_and_comments(value_reference)),
                    ))),
                    skip_ws_and_comments(preceded(
                        tag("DESCRIPTION"),
                        skip_ws_and_comments(cstring),
...
mod tests {
    ...
    use crate::intermediate::{
        constraints::{
            Constraint, ElementOrSetOperation, ElementSetSpecs, SetOperation, SetOperator,
            SubtypeElements,
        },
